...
function buildAssessmentTemplate() {
  const id = "kg2-numbers-class-teacher-assessment-1";
  const templateItems = makeItems(id);
  const maxScore = sumMaxScore(templateItems);

  return {
    id,
    code: "KG2_NUMBERS_CLASS_TEACHER_ASSESSMENT_1",
    title: "الأرقام — المستوى الثاني — معلمة الصف — القياس الأول",
    ...
    source: "create-kg2-numbers-class-teacher-templates",
    createdAt: now,
    updatedAt: now,
  };
}
...
function buildTrackerTemplate() {
  const id = "kg2-numbers-class-teacher-tracker";
  const templateItems = makeItems(id);
  const maxScore = sumMaxScore(templateItems);

  return {
    id,
    code: "KG2_NUMBERS_CLASS_TEACHER_TRACKER",
    title: "الأرقام — المستوى الثاني — معلمة الصف — المتابعة",
...
async function main() {
  ...
  console.log("Creating KG2 Numbers class-teacher templates...");
  console.log({
    orgId,
    dryRun,
    itemMaxScore: ITEM_MAX_SCORE,
...
main().catch((error) => {
  console.error("Failed to create KG2 Numbers class-teacher templates.");
  console.error(error);
  process.exit(1);
});