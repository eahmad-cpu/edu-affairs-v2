...
const TRACKER_ITEMS = [
  "مراجعة الحروف بالفتح",
  "مراجعة حرفين بالفتح",
  "مراجعة كلمات بالفتح",
  "الحروف بحركة الكسر",
  "قراءة حرفين بحركة الفتح والكسر",
  "قراءة كلمات ثلاثية بحركة الفتح والكسر",
  "الحروف بحركة الضم",
  "قراءة حرفين",
  "قراءة كلمات ثلاثية بالحركات الثلاث",
  "السكون المجموعة الاولى حرفين",
  "السكون المجموعة الاولى كلمات",
  "السكون المجموعة الثانية حرفين",
  "السكون المجموعة الثانية كلمات",
];
...
function buildAssessmentTemplate({
  ...
  return {
    ...
    source: "create-kg2-learning-gardens-class-teacher-templates",
    createdAt: now,
    updatedAt: now,
  };
}
...
function buildTrackerTemplate() {
  const id = "kg2-learning-gardens-class-teacher-tracker";
  const templateItems = makeTrackerItems(id);
  const maxScore = sumMaxScore(templateItems);

  return {
    id,
    code: "KG2_LEARNING_GARDENS_CLASS_TEACHER_TRACKER",
    title: "بساتين المعرفة — المستوى الثاني — معلمة الصف — المتابعة",
...
const assessmentTemplates = [
  buildAssessmentTemplate({
    id: "kg2-learning-gardens-class-teacher-pre-assessment",
    code: "KG2_LEARNING_GARDENS_CLASS_TEACHER_PRE_ASSESSMENT",
    title: "بساتين المعرفة — المستوى الثاني — معلمة الصف — القياس القبلي",
  ...
  buildAssessmentTemplate({
    id: "kg2-learning-gardens-class-teacher-assessment-1",
    code: "KG2_LEARNING_GARDENS_CLASS_TEACHER_ASSESSMENT_1",
    title: "بساتين المعرفة — المستوى الثاني — معلمة الصف — القياس الأول",
  ...
  buildAssessmentTemplate({
    id: "kg2-learning-gardens-class-teacher-assessment-2",
    code: "KG2_LEARNING_GARDENS_CLASS_TEACHER_ASSESSMENT_2",
    title: "بساتين المعرفة — المستوى الثاني — معلمة الصف — القياس الثاني",
  ...
  buildAssessmentTemplate({
    id: "kg2-learning-gardens-class-teacher-assessment-3",
    code: "KG2_LEARNING_GARDENS_CLASS_TEACHER_ASSESSMENT_3",
    title: "بساتين المعرفة — المستوى الثاني — معلمة الصف — القياس الثالث",
...
async function main() {
  ...
  console.log("Creating KG2 Learning Gardens class-teacher templates...");
  console.log({
    orgId,
    dryRun,
    learningLossThresholdPercentage: LEARNING_LOSS_THRESHOLD_PERCENTAGE,
...
main().catch((error) => {
  console.error("Failed to create KG2 Learning Gardens class-teacher templates.");
  console.error(error);
  process.exit(1);
});